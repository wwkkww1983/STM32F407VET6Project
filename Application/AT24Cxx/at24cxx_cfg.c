...
UINT8_T AT24CXX_HWI2C_WriteData(AT24CXX_HandlerType *AT24CXXx, UINT16_T addr, UINT8_T *pVal, UINT16_T length)
{
	...
	else
	{
		...
		if (pageByteNum != AT24CXXx->msgAT24CXXPageByte)
		{
			//---读取一页的数据
			#ifndef USE_GOBAL_RAM
			_return = AT24CXX_HWI2C_ReadPageByte(AT24CXXx, (pageIndexAddr * (AT24CXXx->msgAT24CXXPageByte)), pAT24CxxPageByteBuffer, (AT24CXXx->msgAT24CXXPageByte));
			#else
			...
			//---按页写入数据
			#ifndef USE_GOBAL_RAM
			_return = AT24CXX_HWI2C_WritePageByte(AT24CXXx, (pageIndexAddr * (AT24CXXx->msgAT24CXXPageByte)), pAT24CxxPageByteBuffer, AT24CXXx->msgAT24CXXPageByte);
			#else
		...
		if (pageNum != 0)
		{
			for (i = 0; i < pageNum; i++)
			{
				_return = AT24CXX_HWI2C_WritePageByte(AT24CXXx, addr, pVal, AT24CXXx->msgAT24CXXPageByte);
				//---地址偏移，偏移到下次要写入的数据的页的首地址
				addr += AT24CXXx->msgAT24CXXPageByte;
				//---数据偏移
				pVal += AT24CXXx->msgAT24CXXPageByte;
		...
		if (byteNum != 0)
		{
			//---读取一页的数据
			#ifndef USE_GOBAL_RAM
			_return = AT24CXX_HWI2C_ReadPageByte(AT24CXXx, addr, pAT24CxxPageByteBuffer, (AT24CXXx->msgAT24CXXPageByte));
			#else
			...
			//---按页写入数据
			#ifndef USE_GOBAL_RAM
			_return = AT24CXX_HWI2C_WritePageByte(AT24CXXx, addr, pAT24CxxPageByteBuffer, AT24CXXx->msgAT24CXXPageByte);
			#else
